...
var valid_activation_code = false;
    ...
    $.getJSON('/productcode/check_product_code', { code: code_ }, function(data) {
        if (data) {
            ...
            if (data.datetime_used) {
                ...
                */
                for (var i = 1; i < id; i++)
                    if ($('#product-code-text-' + i).val() == code_) used = true;
...
$('.ibo-info-search').click(function() {
    ...
    $.getJSON('/ibo/search', { ibo_id: ibo_id_ }, function(data) {
        ...
        $(".placement_position option[value='L']").show();
        $(".placement_position option[value='R']").show();

        if (data) {
            html = 'IBO Name: ' + data.ibo.info[0].firstname + ' ' + data.ibo.info[0].middlename + ' ' + data.ibo.info[0].lastname;

            if (id_ == 'placement_id') {
                html += '<br/>Left: ' + data.ibo.placement.left + '<br/>Right: ' + data.ibo.placement.right;

                if ((data.ibo.placement.left == 'Free') || (data.ibo.placement.right == 'Free')) {
                    valid_placement = true;

                    if (valid_activation_code) $('.register-button').removeAttr('disabled');
                } else $('.register-button').attr('disabled', true);

                if (data.ibo.placement.left == 'Taken') $(".placement_position option[value='L']").hide();
                if (data.ibo.placement.right == 'Taken') $(".placement_position option[value='R']").hide();
            }
        } else {
